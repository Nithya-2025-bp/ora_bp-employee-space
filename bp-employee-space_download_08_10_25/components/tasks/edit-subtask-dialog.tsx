...
import { updateSubtask } from "@/lib/actions/project-actions"
...
export default function EditSubtaskDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateSubtask(projectId, taskId, subtask.id, title, description, assignedUsers)
      ...
      if (result) {
        ...
        setError("Failed to update subtask. The subtask may not exist in the database.")
      }
    } catch (error) {
      console.error(error)
      if (error instanceof Error) {
        if (error.message.includes("Unauthorized")) {
          setError("You don't have permission to edit subtasks. Only admins can manage tasks.")
          ...
          toast({
            title: "Subtask updated",
            description: "The subtask has been updated in the local data.",
          })
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader className="relative">
            ...
            <DialogDescription>Make changes to your subtask.</DialogDescription>
            <Button
              type="submit"
